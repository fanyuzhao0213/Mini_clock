...
#define THREAD_PRIORITY1         24
#define THREAD_STACK_SIZE1       1024
#define THREAD_TIMESLICE1        5
static rt_thread_t oled = RT_NULL;
...
static void oled_display(void *parameter)
{
    u8g2.begin();
    u8g2.clearBuffer();
    ...
    u8g2.drawStr(30, 60, "By fyzdll");   // write something to the internal memory
    u8g2.sendBuffer();
...
/* 创建线程 */
int oled_thread(void)
{
    /* 创建线程*/
    oled = rt_thread_create("oled_thread",
                             oled_display,
                             RT_NULL,
                            THREAD_STACK_SIZE1,
                            THREAD_PRIORITY1, THREAD_TIMESLICE1);
